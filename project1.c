...
void child_process(int nodeNumber, int nodeCount, int prevNode, int nextNode, char message[100], int whichNode)
{
    ...
    while (1)
    {
        ...
        {
            srand(time(NULL));
            int random_number = rand() % nodeNumber + 1;

            printf("\nPicking a bad apple...\n");
            sleep(1);
            printf("The bad apple is %d\n\n", random_number);
            sleep(2);
            ...
            for (int i = 1; i < nodeNumber + 1; i++)
            {
                if (i == random_number)
                {
                    printf("I am node %d. I am the bad apple. Changing the message...\n", i);
                    sleep(3);
                    // shifts the ascii values of the message by 1. essentially jumbling up the message.
            ...
            printf("I am node %d. I am the destination node. The message was: %s", nodeNumber + 1, message);
